...
void mergeIt(
    ...
    int temp1 = leftStart;
    int temp = leftStart;
    char arr[rightStop+1];
    
    while (leftStart <= leftStop && rightStart <= rightStop) {
        arr[temp++] = (char)findMin((int)data[leftStart], (int)data[rightStart]);
        if((int)data[leftStart] < (int)data[rightStart]){
    ...
    while (leftStart <= leftStop) {
        arr[temp++] = data[leftStart];
        leftStart++;
    }
    while (rightStart <= rightStop) {
        arr[temp++] = data[rightStart];
        rightStart++;
    }
